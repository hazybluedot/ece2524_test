...
part3() {
    ...
    git ls-files mult --error-unmatch 2>/dev/null && ( echo "Do not track binary/compiled files with git" > dkm_review/general.errors; git rm mult 2>/dev/null; git commit -m "remove binary mult" >/dev/null )
